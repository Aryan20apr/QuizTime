...
class Students extends StatefulWidget {
  Students({Key? key, required this.categoryData}) : super(key: key);
...
class _StudentsState extends State<Students> {
  late Future<UserListAlbum> users;
  ...
  Future<UserListAlbum> getStudent() async {
    API api = API();

    return await api.getStudentsOfCategory(widget.categoryData.cid);
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      //   title: Text(
      //     'Your ${widget.categoryData.title} classroom ',
      //     style: TextStyle(
      //         fontSize: 10.sp,
      //         color: Theme.of(context).appBarTheme.foregroundColor),
      //   ),
      // ),
      body: /*(Constants.categories.isNotEmpty&&Constants.quizes.isNotEmpty)?StudentsContent():*/
          FutureBuilder(
...
class StudentsContent extends StatefulWidget {
  StudentsContent({Key? key, required this.users,required this.categoryData}) : super(key: key);
  UserListAlbum users;
  CategoryData categoryData;
  @override
...
class _StudentsContentState extends State<StudentsContent> {
  late Future<UserListAlbum>userlist;
  API api=API();
  void _onRefresh() async{
    // monitor network fetch

    userlist=  api.getStudentsOfCategory(widget.categoryData.cid);
    Future.wait([userlist]).then((value) =>
        setState(()
        {
          widget.users=value[0];}));
                ...
                child: SmartRefresher(
                  ...
                  child: ListView.builder(
                    ...
                    child: Text("Back"),
                      ...
                      UserData userData = widget.users.data![index];
                              ...
                              child: Material(
                                ...
                                child: InkWell(
                                  ...
                                  Navigator.push(context, MaterialPageRoute(builder: (route)=>Students(categoryData: widget.categories.data![index])));*/
                                    ...
                                    child: ListTile(
                                      isThreeLine: true,
                                      leading: Text('${userData.id}',style: TextStyle(color: Theme.of(context).textTheme.displayMedium?.color,fontSize: 15.sp),),
                                      title: Text(
                                          '${userData.firstName} ${userData.lastName}',style: TextStyle(color: Theme.of(context).textTheme.displayMedium?.color)),
                                      subtitle: Text(
                                          'Username: ${userData.nickname} \nEmail: ${userData.email}',style: TextStyle(color: Theme.of(context).textTheme.displayMedium?.color)),
                                      trailing: isEnabledIcon,
                                    )
                                    /*Column(
